...
export default function AddJob() {
	...
	const [eduQuals, setEduQuals] = useState(null)
	const [discipline, setDiscipline] = useState(null)
	const [minWorkExp, setMinWorkExp] = useState(0.0)
	const [maxWorkExp, setMaxWorkExp] = useState(0.0)
	const [jobDesc, setJobDesc] = useState("")
	const [skills, setSkills] = useState("")
	const { employee, isOpen, employeeDispatch } = useEmployeeContext();
	const [reqClientInterview, setReqClientInterview] = useState(false)
	const [project, setProject] = useState(null)
	const [category, setCategory] = useState(null)
	const [TDC, setTDC] = useState(null)
	const [jobStatus, setJobStatus] = useState({ id: 1, name: "Open" })
	...
	const validateForm = async (e) => {
		...
		if (!jobTitle) {
			errorDispatch({ type: 'ERROR', payload: "Select Job Title" })
			setIsLoading(false);
			return;
		}
		...
		// **Convert Work Experience Values to Numbers**
    const minExp = Number(minWorkExp);
    const maxExp = Number(maxWorkExp);

    // **Work Experience Validation**
    if (!minWorkExp || !maxWorkExp || isNaN(minExp) || isNaN(maxExp)) {
    ...
    if (minExp < 0 || maxExp <= 0) {
        errorDispatch({ type: 'ERROR', payload: "Work Experience must be greater than 0" });
        setIsLoading(false);
        return;
    }

    if (maxExp < minExp) {
        errorDispatch({ type: 'ERROR', payload: "Min Work Experience cannot be greater than Max Work Experience" });
        setIsLoading(false);
        return;
    }
		...
		if (!employee) {
			errorDispatch({ type: 'ERROR', payload: "assign not selected" })
			setIsLoading(false);
			return;
		}

		if (!TDC) {
			errorDispatch({ type: 'ERROR', payload: "TDC not selected" })
			setIsLoading(false);
			return;
		}
		if (!project) {
		...
		if (!selectedQuestions || selectedQuestions.length === 0) {
			errorDispatch({ type: 'ERROR', payload: "Questionnaire is required" });
			setIsLoading(false);
			return;
		}

		await axios.post(import.meta.env.VITE_BACKEND_URL + "job/add", { jobTitle: jobTitle, project: project, location: jobLocation, minimumWorkExperience: minWorkExp, maximumWorkExperience: maxWorkExp, qualification: eduQuals, discipline: discipline, category: category, detailedJobDescription: jobDesc, requiredSkillSets: skills, requiredClientInterview: reqClientInterview, tdc: TDC, uploadDate: new Date(), jobStatus: jobStatus, interviewer: employee }, { withCredentials: true })
			.then((response) => {
				if (!response.data) {
					...
					if (selectedQuestions) {
						selectedQuestions.forEach(item => {
							axios.post(import.meta.env.VITE_BACKEND_URL + "default-questionnaire/add", { question: item, job: response.data }, { withCredentials: true })
								.catch(() => {
									errorDispatch({ type: 'ERROR', payload: "Error adding Questionnaire" })
								})
						})
					}

					if (employee) {
						axios.post('${import.meta.env.VITE_BACKEND_URL}mail/send',
							{
      ...
      `
							...
							.catch(() => { errorDispatch({ type: 'ERROR', payload: "Failed to send email" }); setIsLoading(false); })
					}
				}
			})
			.catch(() => { errorDispatch({ type: 'ERROR', payload: "Error creating job" }); setIsLoading(false) })
		...
		<AnimatePresence>
			...
			{showOverlay &&
				<motion.div key="addOverlay" className="absolute z-10 top-0 left-0 w-full h-full backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
					<OverlayInput label={overlayLabel} overlayUrl={overlayUrl} setShowOverlay={setShowOverlay} trigger={trigger} setTrigger={setTrigger} maxLen={maxLen} />
				</motion.div>
			}
			{showQuestionOverlay &&
				<motion.div key="addOverlay" className="absolute z-10 top-0 left-0 w-full h-full backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
			...
			{showJobOverlay &&
				<motion.div key="jobOverlay" className="absolute z-10 top-0 left-0 w-full h-full backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
					<OverlayJobInput setShowOverlay={setShowJobOverlay} trigger={trigger} setTrigger={setTrigger} setJobTitle={setJobTitle} />
				</motion.div>
			}
			<form className="w-3/4 h-full flex justify-center items-start py-8" id="jobDetails" onSubmit={validateForm}>
				<div className="flex flex-col justify-start items-start w-2/3 min-h-full max-h-full pb-10 ">
					...
					<div className="flex w-full justify-between items-end">
						...
						<div className="w-1/2 mx-2"><SelectInput id="jobLocation" label="Job Location" form="jobDetails" setFunction={setJobLocation} list={locationsList} addFunction={addLocation} /></div>
					</div>

					<div className="flex w-full justify-between items-end">
						<div className="w-5/12"><SelectInput id="eduQuals" label="Educational Qualifications" form="jobDetails" setFunction={setEduQuals} list={qualsList} addFunction={addQual} /></div>
						<div className="w-5/12 mx-2"><SelectInput id="discipline" form="jobDetails" label="Discipline" setFunction={setDiscipline} list={disciplineList} addFunction={addDiscipline} /></div>
					...
					<div className="flex w-full justify-between items-end">
						<div className="w-full mr-2">
							<TextAreaInput id="jobDesc" label="Job Description" setFunction={setJobDesc} maxLen={2000} value={jobDesc} />
						</div>
					</div>
				...
				<div className="flex flex-col justify-evenly items-center w-1/3 min-h-full max-h-full pb-10 ">
					...
					<div className="flex w-full justify-center items-center">
						<input id="clientReq" type="checkbox" onChange={(e) => { setReqClientInterview(e.target.checked) }} />
					...
					<div className="flex flex-col w-1/2">
						...
						<input
							id="TDC"
							type="date"
							className="py-1 px-3 rounded-sm bg-theme_gray border border-gray-300 focus:border-red-500"
					...
					<div className="flex w-3/4 justify-center items-center">
						<SelectInput id="project" label="Project" form="jobDetails" setFunction={setProject} list={projectList} addFunction={addProject} />
					</div>

					<div className="flex w-3/4 justify-center items-center">
						<SelectInput id="category" label="Category" form="jobDetails" setFunction={setCategory} list={categoryList} addFunction={addCategory} />
					</div>
