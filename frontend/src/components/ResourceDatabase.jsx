            ...
            const fetchData = async () => {
                try {
                    if (employee.userType.id != 2) {
                        await axios.get (import.meta.env.VITE_BACKEND_URL+"candidate/getAll", {withCredentials: true})
                        .then (response => {
                            if (response.data) {
                        ...
                        await axios.get (import.meta.env.VITE_BACKEND_URL+`candidate/getByEmployee/${employee.synergizId}`, {withCredentials: true})
                        .then (response => {
                            if (response.data) {
                                setCandidates (response.data)
                                setIsLoading (false)
